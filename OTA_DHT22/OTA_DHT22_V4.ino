...
// MQTT config
const char* mqtt_server = "192.168.15.119";
...
const char* NODE_LOCATION = "sawah1";
const char* NODE_TYPE = "DHT22";
const char* NODE_ID = "Node-DHT";
const char* CURRENT_VERSION = "1.4";

WiFiClient wifiClient;
PubSubClient client(wifiClient);
DHT dht(DHTPIN, DHTTYPE);

// === Publish OTA or system log ===
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == topic_ota) {
    ...
    const char* targetLocation = doc["location"];
    const char* targetType = doc["type"];
    const char* url = doc["url"];
    const char* targetVersion = doc["version"]; // optional

    if (!targetLocation || !targetType || !url) {
      publishJsonLog("ota", "❌ Incomplete OTA payload");
      return;
    }

    if (strcmp(targetLocation, NODE_LOCATION) == 0 && strcmp(targetType, NODE_TYPE) == 0) {
